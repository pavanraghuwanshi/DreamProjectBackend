...
export const addSchool = async (req, res) => {
  ...
  try {
    ...
    const newSchool = new School({ schoolName, username, password: encryptedPassword, email, schoolMobile, branchName, fullAccess });
...
export const deleteSchool = async (req, res) => {
  ...
  try {
    ...
    await Child.deleteMany({ schoolId: id });
...
export const grantSchoolAccess = async (req, res) => {
  const role = req.user.role;
  if (role !== "superAdmin") {
    return res.status(403).json({ message: "You are not authorized to change access" });
  }
  ...
  try {
    ...
    if (typeof fullAccess !== "boolean") {
      return res.status(400).json({ message: "Invalid value for fullAccess. Must be true or false." });
    }
...
export const addMultipleSchools = async (req, res) => {
  const role = req.user.role;
  if (role !== "superAdmin") {
    return res.status(403).json({ message: "You are not authorized to add schools" });
  }
...
const schools = xlsx.utils.sheet_to_json(sheet);
    ...
    for (const school of schools) {
      const { schoolName, username, password, email, schoolMobile, branchName, fullAccess } = school;
